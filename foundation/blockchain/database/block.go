...
func POW(ctx context.Context, args POWArgs) (Block, error) {
	...
	tree, err := merkle.NewTree(args.Trxs)
	if err != nil {
		return Block{}, err
	}
...
func (b Block) ValidateBlock(previousBlock Block, stateRoot string, evHandler func(v string, args ...any)) error {
	...
	if b.Header.TrxRoot != b.MerkleTree.RootHex() {
		return fmt.Errorf("merkle root does not match transactions, got %s, exp %s", b.MerkleTree.RootHex(), b.Header.TrxRoot)
	}
